...
LOOKAHEAD_WPS = 200 # 200 Default; Number of waypoints we will publish. You can change this number
...
class WaypointUpdater(object):
    ...
    def waypoints_cb(self, msg):
        rospy.loginfo("Base waypoints received %s", msg)
    ...
    def traffic_cb(self, msg):
        ...
        rospy.loginfo("traffic light received with index: %s" , self.traffic_waypoint_index)
    ...
    def get_closest_waypoint(self, current_pose):
        #rospy.loginfo("Base waypoints: %s", self.base_waypoints[1])
        ...
        for i in range(len(self.base_waypoints)):
            ...
            if (dist < min_dist):
                min_dist = dist
                index_min_dist = i 
        rospy.loginfo("Minimal distance to waypoint with index %s is: %s", index_min_dist , min_dist)
        self.index_closest_waypoint = index_min_dist

    def set_final_waypoints(self):
        number_of_rear_waypoints=0
        #rospy.loginfo("Final waypoints index closest point: %s", self.index_closest_waypoint+LOOKAHEAD_WPS)
    ...
    def set_speed_adapted_base_waypoints(self):    
        ...
        if self.traffic_waypoint_index >=0:
            ...
            rospy.loginfo("start speed: %s at index: %s", start_speed, start_index)
            ...
            for i in range(start_index, self.traffic_waypoint_index):
                rospy.loginfo("i: %s, index_range: %s, speed_range: %s",i,index_range,speed_range)
                self.speed_adapted_base_waypoints[i].twist.twist.linear.x=np.interp(i,index_range,speed_range) 
                rospy.loginfo("interpolation result: %s at index: %s", np.interp(i,index_range,speed_range),i)
